...
Line 30, Primary dataset imported 

Line 36, Sub dataset imported 

(B) .Rmd Line numbers for inspecting data intake:  
Line 43, Primary dataset inspection with ‘str’ 

Line 48, Subset dataset inspection with ‘str’ 
...
Line 59, Use filter function to find ‘US’ in the primary dataset 

Line 77, Use group_by function to group ‘State’ for visualization 

Line 78, Use summarise function to figure out ‘sum’ in the primary dataset 

Line 79, Use arrange function to order ‘Death’ in descending order        

Line 98, Use mutate function to change the variable name with regex. 

(B) .Rmd Line number(s) for a join operation: 

 Line 102, Join the datasets by ‘inner_join’ which are already cleaned 
...
Line 98, Use “, .+”  as a pattern to change the data into different format 

(E) .Rmd Line number(s) for use of reduction and/or transformation functions: 

Line 78, Use sum function to find total of Confirmed, Recovered, and Deaths

(F) .Rmd Line number(s) for use of user-defined functions: 

Line 66, Use ‘round’ function to remove decimal points on ‘AirQualityIndex’ 
...
Line 142, Use geom_point and geom_line to generate scatter plot for Confirmed vs Airquality 

Line 148, Use Use geom_point and geom_line to generate scatter plot for Recovored vs Airquality 

Line 155, Use geom_point and geom_line to generate scatter plot for Deaths vs Airquality 

(B) .Rmd Line number(s) for use of multiple aesthetics:  

Line 142, 148, 155 Use colour and x, y position aesthetic to show the data by each States 

(C) .Rmd Line number(s) for use of layered graphics:  

(D) .Rmd Line number(s) for use of leaflet maps:  

Line 82, Use usmap to show Confirmed population in U.S 

Line 95, Use usmap to show Recovered population in U.S 

Line 107, Use usmap to show Deaths population in U.S 

Line 121, Use usmap to show Airquality in U.S 
